...
async function carregarDadosDaAPI() {
  ...
  try {
    const [maquinas, produtos] = await Promise.all([
      requisitarAPI("/maquinas"),
      requisitarAPI("/produtos")
    ]);
    dados.maquinas = maquinas.map(item => ({
    ...
    apiErro = erro.message;
    mostrarMensagem(`Não foi possível carregar a API: ${erro.message}`, "erro");
  } finally {
    apiCarregando = false;
    navegar(paginaAtual);
  }
...
function paginaMaquinas() {
  if (apiCarregando) return `<section class="cartao"><h2>Carregando máquinas...</h2><p>Buscando dados reais na API.</p></section>`;
  if (apiErro) return `<section class="cartao"><h2>Não foi possível carregar as máquinas</h2><p>${apiErro}</p><button class="botao-primario" data-acao="recarregar-api">Tentar novamente</button></section>`;
...
function tratarAcao(acao, id, elemento) {
  ...
  if (acao === "recarregar-api") carregarDadosDaAPI();
}
...
async function excluirItem(lista, id, nome) {
  ...
  if (lista === "maquinas" && !modoHospedado) {
    try {
      ...
      return mostrarMensagem(erro.message, "erro");
    }
  }
...
async function salvarMaquina(formulario) {
  ...
  try {
    ...
    maquina.id = Number(maquina.id);
    maquina.temperatura = Number(maquina.temperatura);
    maquina.eficiencia = Number(maquina.eficiencia);
    if (id) dados.maquinas[dados.maquinas.findIndex(item => item.id === id)] = maquina;
    else dados.maquinas.push(maquina);
    fecharModal();
    navegar(paginaAtual);
    mostrarMensagem("Máquina salva com sucesso.");
  } catch (erro) {
    mostrarMensagem(erro.message, "erro");
    botao.disabled = false;
    botao.textContent = "Salvar máquina";
  }
}
